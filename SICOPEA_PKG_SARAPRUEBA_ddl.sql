...
procedure save_transferencia
    ...
    begin
        ...
        if codSol=0 then
            ...
            p_nroRaTra:='0';
            --p_nroRaTra:=pkg_saraprueba.GENERARRESOLUCIONTRANSFERENCIA;
...
procedure aprobar_transferencia
    ...
    begin
        ...
        if p_codEstadoSol='A' then
            vnrora:=pkg_saraprueba.GENERARRESOLUCIONTRANSFERENCIA;
        else
            vnrora:='0';
        end if;
        p_ra_trans:=vnrora;
...
procedure validarTransferencia
    ...
    begin
        ...
        select count(*) into vContTrans from solicitud so, ops$asy.sad_trr@basy.sidunealin ve,ops$asy.sad_gen@basy.sidunealin sa where SO.COD_FRV=ve.sad_att_ref
        ...
        if vnrofrv!='020000596' then
            select count(to_char(sa.sad_reg_date+(vNroDias+1),'dd/mm/yyyy')) into contFch from solicitud so, ops$asy.sad_trr@basy.sidunealin ve,ops$asy.sad_gen@basy.sidunealin sa where SO.COD_FRV=ve.sad_att_ref
            and VE.KEY_YEAR=sa.key_year and ve.key_cuo=sa.key_cuo and ve.key_dec=sa.key_dec and ve.key_nber=sa.key_nber
            and so.ULT_VER=0 and so.LST_OPE='U' and so.cod_tipo_bien='2' and so.nro_ra_sol=p_nroRa
            and ve.sad_num=0 and sa.lst_ope='U' and sa.sad_num=0 and ve.sad_att_cod='996'
            and sa.sad_typ_proc=4 and sa.sad_flw=1 and sa.sad_asmt_nber is not null;
            ...
            if contFch>0 then
                select to_char(sa.sad_reg_date+(vNroDias+1),'dd/mm/yyyy') into vFechaTrans from solicitud so, ops$asy.sad_trr@basy.sidunealin ve,ops$asy.sad_gen@basy.sidunealin sa where SO.COD_FRV=ve.sad_att_ref
                and VE.KEY_YEAR=sa.key_year and ve.key_cuo=sa.key_cuo and ve.key_dec=sa.key_dec and ve.key_nber=sa.key_nber
                and so.ULT_VER=0 and so.LST_OPE='U' and so.cod_tipo_bien='2' and so.nro_ra_sol=p_nroRa
                and ve.sad_num=0 and sa.lst_ope='U' and sa.sad_num=0 and ve.sad_att_cod='996'
                and sa.sad_typ_proc=4 and sa.sad_flw=1 and sa.sad_asmt_nber is not null;
